...
class Model:
    ...
    def __init__(self, wires, batch_size):
        '''
        4 + 2 qubit system
        for 4 bit integer classification
        '''
    ...
    def circuit(x, theta):
        ...
        for wire in (data[2:] + combined):
            qml.Rot(*theta[wire], wires=wire)
        for wire in (data[2:] + combined):
            qml.CNOT([wire, (wire+1) % len(data[2:]+combined)])

        return qml.expval(qml.PauliZ(combined[1]))

    def draw(self):
        qml.draw_mpl(Model.circuit)(2, np.ndarray((6, 3)))
        plt.show()
